...
def load_data():
    ...
    import os
    try:
        # Determinar la ruta correcta dependiendo de dónde se ejecute
        # Si está en version_final_corregida/, buscar matches_cleaned.csv en el mismo directorio
        # Si está en raíz (Streamlit Cloud), buscar en version_final_corregida/
        script_dir = os.path.dirname(os.path.abspath(__file__))
        csv_path = os.path.join(script_dir, 'matches_cleaned.csv')
        
        # Si no existe en el directorio actual, intentar desde raíz
        if not os.path.exists(csv_path):
            csv_path = os.path.join(os.path.dirname(script_dir), 'version_final_corregida', 'matches_cleaned.csv')
...
def load_model_metrics():
    ...
    import os
    try:
        # Determinar la ruta correcta para el archivo de métricas
        script_dir = os.path.dirname(os.path.abspath(__file__))
        metrics_path = os.path.join(script_dir, 'metrics_summary_real.json')
        
        # Si no existe en el directorio actual, intentar desde raíz
        if not os.path.exists(metrics_path):
            metrics_path = os.path.join(os.path.dirname(script_dir), 'version_final_corregida', 'metrics_summary_real.json')
